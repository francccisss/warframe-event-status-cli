...
async function logCurrentEventStatus(data, wfEvents) {
...
function createFlatTable(data, type = "vert") {
    const relevantData = [
        "state",
        "timeLeft",
        "location",
    ...
    const reducedDataKeys = Object.keys(reducedData).map((name) => name.toUpperCase());
    if (type === "hor") {
        ...
        const newTable = Object.values(reducedData);
        table.push(newTable);
        if (nestedData !== null) {
...
function createNestedTable(nestedTableArray) {
    const relevantData = [
        "modifierDescription",
        "modifier",
        "missionType",
    ...
    const nestedArray = nestedTableArray[nestedTitle];
    ...
    nestedArray.forEach((value, i) => {
        ...
        for (let j = 0; j < Object.values(reducedData).length; j++) {
            Object.values(reducedData)[j] !== null
                ? tmpArr.push(Object.values(reducedData)[j].toString())
...
function extractRelevantData(data, relevantData) {
    let reducedData = {};
    let nestedData = null;
